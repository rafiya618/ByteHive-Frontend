...
const CommunityFormCard = () => {
  ...
  // Load edit data or draft on component mount
  React.useEffect(() => {
    if (isEdit) {
      setFormData({
        community_name: editCommunity.community_name || '',
        description: editCommunity.description || '',
        community_tags: editCommunity.community_tags || [],
        visible: editCommunity.visible || 'public',
        moderation: editCommunity.moderation || 'only admin'
      });
      setTagsInputValue((editCommunity.community_tags || []).join(', '));
      setImagePreview(editCommunity.image || null);
      return;
    }
    ...
    <div className="bg-navbar-bg border border-navbar-border rounded-2xl p-6 flex flex-col gap-6 w-full">
      ...
      {success && (
        <div className="bg-green-600/20 border border-green-600/30 rounded-lg p-4 text-green-400">
          {isEdit ? 'Community updated successfully! Redirecting...' : 'Community created successfully! Redirecting to communities...'}
        </div>
      )}

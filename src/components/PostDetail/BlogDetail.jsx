...
export default function BlogDetail() {
  ...
  const currentUserId = auth?.user?._id ?? auth?.user?.id ?? auth?.user?.user_id ?? auth?.user?.sub ?? auth?.user?.userId ?? null;
                        ...
                        try {
                          if (post?.community) {
                            ...
                            const matched = Array.isArray(list)
                              ? list.find(c => String(c?.community_name || '').toLowerCase() === String(post.community).toLowerCase())
                              : null;
                            if (matched?._id) {

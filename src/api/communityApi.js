...
export const communityApi = {
  ...
  // Add Post to Community (Protected)
  addPostToCommunity: async (communityId, postId) => {
    try {
      if (!communityId || !postId) throw new Error('communityId and postId are required');
      const user_id = getUserIdFromAuth();
  ...
  // Remove Post from Community (Protected)
  removePostFromCommunity: async (communityId, postId) => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/communities/${communityId}/posts/${postId}`, {
        ...
        // Express can read DELETE body; backend expects user_id in body
  ...
  updateCommunity: async (communityId, communityData, imageFile = null) => {
    try {
      ...
      // Attach current user id so backend can authorize the update (backend expects user_id)
      try {
        const currentUserId = getUserIdFromAuth();
        formData.append('user_id', currentUserId);
      } catch (e) {
        console.warn('Could not attach user_id to update request:', e.message);
      }
  ...
  followCommunity: async (communityId) => {
    try {
      // include user_id in the request body as the backend requires it
      const user_id = getUserIdFromAuth();
  ...
  getUserCommunities: async (searchQuery = '', forceRefresh = false) => {
    try {
      ...
      // Build URL with the actual user id; backend route uses :userId but controller expects user_id,
  ...
  deleteCommunity: async (communityId) => {
    try {
      // backend expects user_id in the request body for authorization
      let body = null;
      try {
        const user_id = getUserIdFromAuth();
        body = JSON.stringify({ user_id });
      } catch (e) {
        console.warn('No user_id available for delete request:', e.message);
      }
